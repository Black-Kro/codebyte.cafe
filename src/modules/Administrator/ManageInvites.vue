<template>
    <app-default-layout>
        <kro-surface :padded="false">
            <kro-list-item>
                <template #icon><kro-icon icon="add" /></template>
                Create Invites
                <template #subtitle>Create an invite for new users</template>
                <template #action>
                    <kro-dialog v-model="isDialogOpen">
                        <template #activator="{ open }">
                            <kro-button @click="open">Create</kro-button>
                        </template>
                        <template #default>
                            <span class="font-bold text-lg">Create Invite</span>
                            <p class="text-secondary text-sm pt-2 max-w-sm">
                                Create a new invite to send to users. Below you can select roles that will be automatically applied to user who
                                join with the invite code as well as how many invites you would like to create.
                            </p>

                            <div class="[ create-invite ] py-4">
                                <div class="text-sm text-secondary font-bold">Roles</div>
                                <div class="flex flex-col">
                                    <kro-switch v-model="roles" value="Moderator">Moderator</kro-switch>
                                    <kro-switch v-model="roles" value="Admin">Admin</kro-switch>
                                </div>
                            </div>
                            <div>
                                <div class="text-sm text-secondary font-bold pb-2">Amount to create</div>
                                <kro-slider 
                                    ticks
                                    v-model="amount" 
                                    :steps="1"
            ...
            <div class="py-4">
                <div class="px-4 text-sm font-bold text-secondary">Exisiting Invits</div>
                <kro-list-item v-for="invite in invites" :key="invite.id">
                    {{invite.code}}
                    <template #subtitle>
                        <span>Created by @{{invite.createdBy.username}}</span>
                        <span class="mx-2">·</span>
                        <span>Roles: {{invite.initialRoles.length > 0 ? invite.initialRoles.join(', ') : 'None'}}</span>
                        <span class="mx-2">·</span>
                        <span>Created {{format(invite.createdAt)}}</span>
                    </template>
                    <template #action>
                        <kro-button @click="deleteInvite(invite.id)" icon="delete" />
                    </template>
                </kro-list-item>
            </div>
        </kro-surface>
...
<script lang="ts" setup>
    ...
    import { CREATE_INVITES } from '/~/apollo/mutation';
...
<style lang="scss">
    ...
    .create-invite {
        --kro-switch-track-background: rgba(0, 0, 0, .24);
    }
