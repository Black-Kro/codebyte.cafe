<template>
  <div
    class="container mx-auto h-full p-4 flex flex-col items-center justify-center"
  >
    <kro-surface class="max-w-sm w-full">
      <kro-avatar
        class="w-24 h-24 mx-auto"
        :src="`https://avatars.dicebear.com/api/bottts/codebyte.svg`"
      />

      <form class="grid py-2" @submit.prevent="createAccount">
        <kro-textfield
          v-model="username"
        ...
        <kro-textfield v-model="bio" label="Bio" multiline />

        <kro-textfield v-model="inviteCode" class="mt-4" label="Invite Code" />
    ...
    <kro-alert v-if="error.length > 0" class="mt-4" type="error">
      {{
        error
      }}
    </kro-alert>
    <div class="p-4 pb-0 max-w-sm w-full text-center text-sm">
      By creating an account you are agreeting to our Privacy Policy and our
      Terms of Service.
    </div>
...
export const {
  ...
  error: usernameError,
  loading: isUsernameLoading,
} = useFormValidation('/api/validation/username/', username, 'Username')

export const {
  error: nicknameError,
  valid: isNicknameValid,
  loading: isNicknamLoading,
} = useFormValidation('/api/validation/nickname/', nickname, 'Nickname')
...
export const createAccount = async() => {
  ...
  try {
    ...
    if (getters['auth/status'] === 'AUTHENTICATED') { replace({ path: '/' }) }
    else {
      createDialog({
        title: 'Oops',
        message:
          'Uh oh, there was an error creating your account. Please try again later. If this issue persists, pleace contact the admins for help',
        icon: 'mdi:alert',
        style: 'centered',
