<template>
    <div class="container mx-auto h-full p-4 flex flex-col items-center justify-center">
        <kro-surface class="max-w-sm w-full">
            ...
            <form @submit.prevent="signup" class="grid gap-4 py-2">
                <kro-textfield 
                    id="email" 
                    v-model="email"
                    required
                ...
                <kro-button type="submit" :loading="loading" primary>Sign Up</kro-button>
            ...
            <div class="grid gap-2 py-2" style="--kro-button-background: var(--kro-background)">
                <kro-button @click="signin('GOOGLE')"><kro-icon icon="google"/>Sign Up with Google</kro-button>
                <kro-button @click="signin('GITHUB')"><kro-icon icon="github"/>Sign Up with Github</kro-button>
            </div>
        </kro-surface>
...
<script lang="ts" setup>
    ...
    export const signup = async () => {
        ...
        await dispatch('auth/signUp', { email: email.value, password: password.value });
