...
<script lang="ts" setup>
    ...
    const { mutate: updateProfile } = useMutation<any, { header: string }>(UPDATE_PROFILE);
    ...
    export const onChange = async ({ target }) => {
        ...
        try {
            // Crop Media
            const cropped = await createCropper(files[0], { aspectRatio: 16 / 9 }, { maxWidth: 1920, maxHeight: 1080 });
