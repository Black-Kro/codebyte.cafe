<template>
  <div>
    <user-me>
      <template #me="{ me }">
        <div
          class="[ avatar-uploader ] [ relative w-32 h-32 ] [ rounded-full overflow-hidden ]"
        >
          <user-avatar
            :user="me"
            :size="128"
          ...
          />
          <kro-spinner v-if="isLoading" class="w-32 h-32 absolute top-0 left-0" />
    ...
    <span class="text-center text-xs text-foregroundSecondary font-bold block mt-2">
      Max File Size: 5MB
    </span>
  </div>
</template>
...
const { mutate: updateProfile } = useMutation<any, { avatar: string }>(
  UPDATE_PROFILE,
)
