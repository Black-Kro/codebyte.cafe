<template>
  <div>
    <div
      class="app-post flex flex-row"
      :class="{ 'rounded-md': post.level > 0 }"
    >
      <div class="pt-3 pl-4 flex flex-col items-center">
        ...
        <div v-if="post.childCount > 0 && post.parent" class="flex-1 py-1">
          <div
            class="post-trail cursor-pointer"
      ...
      <div class="flex-1">
        <div class="flex-1">
          ...
          <div v-if="post.text" class="[ post-renderer-content ]">
            <app-post-renderer :content="post.text" />
          </div>

          <div
            v-if="post.media && post.media.length > 0"
            :class="{ 'mt-4': post.text }"
            class="m1-2 rounded-md overflow-hidden mr-4"
          >
            <app-post-media :media="post.media" />
          </div>
          ...
          >
            <app-post-gif :gifs="post.externalMedia" />
          </div>
