<template>
  <div v-if="files.length > 0" class="[ app-post-box__images ] [ p-4 ]">
    <div class="grid grid-cols-4 gap-4">
      <div v-for="file in files" :key="file" class="relative">
    ...
    <div class="flex flex-row items-center pt-4">
      <div class="flex-1" />
      <span
        class="text-foregroundSecondary text-xs font-medium mr-1"
      >{{ totalSize }}MB / 5MB</span>
      <app-progress-ring
        :color="totalSize > 5 ? 'var(--kro-error)' : 'var(--kro-foreground)'"
        :stroke="4"
        :radius="16"
        :progress="(totalSize / 5) * 100"
      />
    </div>
  </div>
</template>
...
export const totalSize = computed(() => {
  return (
    props.files.reduce((count, o) => count + o.size, 0)
    / (1024 * 1024)
  ).toFixed(2)
})
...
export const editFile = async(file: File) => {
  try {
    ...
    emit(
      'update:files',
      props.files.map(f => (f === file ? cropped : f)),
    )
  }
  catch (error) {}
}
